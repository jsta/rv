...
\bibitem[Gelman et~al.(2003)Gelman, Carlin, Stern, and Rubin]{BDA}
Andrew Gelman, John~B. Carlin, Hal~S. Stern, and Donald~B. Rubin.
\newblock \emph{Bayesian Data Analysis}.
\newblock Chapman \& Hall/CRC, London, 2nd edition, 2003.

\bibitem[Martin and Quinn(2004)]{MarQui04}
Andrew~D. Martin and Kevin~M. Quinn.
\newblock \texttt{MCMCpack} 0.5-2.
\newblock \url{http://mcmcpack.wustl.edu/}, 2004.

\bibitem[Plummer(2004)]{JAGS}
Martyn Plummer.
\newblock {JAGS}: {J}ust {A}nother {G}ibbs {S}ampler.
\newblock \url{http://www-fis.iarc.fr/$\sim$ \!martyn/software/jags/}, 2004.

\bibitem[{R Development Core Team}(2004)]{Rproject}
...
\bibitem[Thomas and O'Hara(2004)]{OpenBUGS}
Andrew Thomas and Robert~B. O'Hara.
\newblock {OpenBUGS}.
\newblock \url{http://mathstat.helsinki.fi/openbugs/}, 2004.
